/*
  ...
  Course: cse2010
  Section: 23
  Description: Homework2EXTRA - Class schedule arrangement without recursion
...
public class HW2Extra
{
    ...
    static void runPermutations() {
    		...
    		String[] possibleStringList = withoutRecursion(indexArray);//array with all possibilities
    		for(int count = 0; count < possibleStringList.length; count++) {
    			int[] temp = new int[courses.size()];
    			Scanner pointer = new Scanner(possibleStringList[count]);
    			for(int i = 0; pointer.hasNext() && i < temp.length; i++) {
    				temp[i] = pointer.nextInt(); 
    			}
    			
    			Arrays.sort(temp); //next algorithm requires them to be in order
    ...
    /*
     * Checks if multiple classes are on the same schedule. Does not add them in
     * Adds in all the ones that are valid
     */
    static void checkDoubles(int[] temp) {
    		//check after temp array is full
    			...
    			if(valid) {
    				...
    				int[] newtemp = new int[courses.size()];
    ...
    /*
     * Takes input of all possible indexes as array
     * returns array of string with all possiblities. each index is seprated by space
     */
    static String[] withoutRecursion(int[] indexes) {
    		int length = indexes.length;
    		int total = ((Double) Math.pow (2, length)).intValue () - 1; //total amount of combinations
    		
    		
    		TreeSet<String> list = new TreeSet<String>(); //adds everything to a tree set
    		for (int countAll = 0; countAll < total; countAll++) {
            String tmp = "";
            
            char[] charArray = new StringBuilder (Integer.toBinaryString (countAll)).reverse ().toString ().toCharArray ();
