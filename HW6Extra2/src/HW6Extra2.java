...
public class HW6Extra2
{
	...
	static arrayGraphExtra2 graph;				//playing graph
		...
		while(true) {
			...
			for(int i = 0; i < bugs.size(); i++) {
				...
				//list is returned with only one thing if bug is where the player is
					...
					bugsWin = true;
					System.out.println("Bug " + bugs.get(i) + " Wins!");
					break;
    ...
    static void getGraph(Scanner scanner) {
	    	...
	    	graph = new arrayGraphExtra2(xValue, yValue);
	...
	static LinkedList<Vertex> bugMovement(Vertex bug) {
		LinkedList<Coordinates> coords = graph.BFS(player.coordnates, exit.coordnates);
